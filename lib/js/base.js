...
function select_item(idx, url, id) {
	modal_close();
	w3_close();
	close_all_xs_menu();
	common(url, id);
...
function common(entry, id) {
	id = (id == undefined ? (entry.length == 1 ? ["main"] : ["main", "myst"]) : id);
	if((typeof id) == "object" && (typeof entry) == "object" && id.length == entry.length) {
		if(entry.length == 1) {
			request(entry[0], id[0]);
		} else if(entry.length == 2) {
			request(entry[0], id[0]);
			request(entry[1], id[1]);
		}
	}
}
