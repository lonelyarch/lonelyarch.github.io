...
function fix_sidemenu() {
    ...
    if (w < 993 && w > 600) {
        ...
        if (top > 0 && top < 100) {
            ...
            document.getElementById("searcharticles").style.top = (144 - top - 32) + "px";
        }
        if (top > 100) {
...
function search_open(event) {
    ...
    if(document.getElementById("topnav").style.position == "fixed") {
    	...
    	var top = scrolltop();
        document.getElementById("searcharticles").style.top = "112px" - top;
    }
...
function search_close() {
    var x = document.getElementById("searcharticles");
    x.className = x.className.replace(" w3-show", "");
    var y = document.getElementById("topnav").getElementsByTagName("ul")[1].getElementsByTagName("a")[0];
    if(y.className.indexOf("fa-times") == -1) {}
    else {y.className = y.className.replace(" fa-times", " fa-search");}
}
...
function select_list(idx, obj) {
	if((typeof obj) === "object") {
		if(obj.url === undefined || obj.id === undefined) {}
        else {
        	var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    document.getElementById(obj.id).innerHTML = this.responseText;
                    document.getElementById("list").getElementsByTagName("a")[idx].className = "w3-text-blue";
                } else {
        ...
        y[i+2].addEventListener("click", function() {
            ...
            for(var j=0; j<len; j++) {
                if(y[j+2].className === "active") {
                	...
                	request("src/articles/initial/css/" + j + ".css", "myst");
                	request("src/articles/initial/" + j + ".txt", "main");
                }
            }
        });
    }
